...
function StockReturn() {
  ...
  const generateUniqueReturnNumber = () => {
    ...
    currentReturns.forEach(return_ => {
      const match = return_.returnNumber.match(/^STR-(\d+)$/)
      if (match) {
    ...
    return `STR-${randomNumber.toString().padStart(5, '0')}`
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (editingReturn) {
      ...
      const updatedReturns = stockReturns.map(r => 
  ...
  const handlePrintReturn = (return_) => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>Stock Return - ${return_.returnNumber}</title>
          <style>
            body {
        ...
        <body>
          ...
          <div class="invoice-info">
            <div class="info-section">
              ...
              <p><strong>Supplier:</strong> ${return_.supplierName || 'N/A'}</p>
            </div>
          </div>
          <table>
            <thead>
  ...
  const handleEditReturn = (return_) => {
    ...
    setFormData({
      ...
      supplierName: return_.supplierName,
      date: return_.date,
  ...
  const filteredReturns = stockReturns.filter(return_ => {
    const searchLower = searchTerm.toLowerCase()
    return (
      return_.returnNumber.toLowerCase().includes(searchLower) ||
      return_.supplierName.toLowerCase().includes(searchLower) ||
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <div style={{ position: 'relative', display: 'inline-block' }}>
            <input
              ...
              placeholder="Search by return number, supplier, date, or items..."
              value={searchTerm}
        ...
        <div className="modal">
          <div className="modal-content" style={{ maxWidth: '700px' }}>
            <div className="modal-header">
              <h2 className="modal-title">{editingReturn ? 'Edit Stock Return' : 'New Stock Return'}</h2>
      ...
      {viewingReturn && (
        <div className="modal" onClick={() => setViewingReturn(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '700px', width: '95%' }}>
            <div className="modal-header">
              <h2 className="modal-title">Stock Return - {viewingReturn.returnNumber}</h2>
            ...
            <div style={{ padding: '1rem 0' }}>
              <div style={{ marginBottom: '1.5rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
                  ...
                  <div>
                    <strong>Supplier:</strong> {viewingReturn.supplierName}
                  </div>
                </div>
